...
class KdTree:
    ...
    def points_in_range(self, query_rectangle):
        ...
        if self.is_leaf:
            ...
            horiz_mid = (query_rectangle[0][0] + query_rectangle[1][0]) / 2
            vertical_mid = (query_rectangle[0][1] + query_rectangle[1][1]) / 2
            matches = []
            if :
                matches += points_in_range(self.leftorbottom, query_rectangle)
            if:
                matches += points_in_range(self.rightortop, query_rectangle)
        return matches
